-- Schritt 10d: admin_close_payroll_period() (Migration 20260721091511, Schritt 10c) soll pro
-- erzeugtem payroll_snapshot auch einen aggregierten payroll_cost-Eintrag in financial_events
-- schreiben, damit RevenueCostChart eine einzige Ledger-Quelle fuer Einnahmen UND Kosten hat.
-- Die Funktion ist SECURITY INVOKER und laeuft als der aufrufende Admin -- financial_events erlaubt
-- authenticated bisher aber nur SELECT (Migration 20260721095032), Schreiben lief bewusst nur ueber
-- SECURITY-DEFINER-Trigger. Admin-INSERT wird deshalb hier gezielt ergaenzt (weiterhin kein
-- UPDATE/DELETE fuer irgendeine Rolle -- der Ledger bleibt append-only).

GRANT INSERT ON TABLE public.financial_events TO authenticated;

CREATE POLICY financial_events_admin_insert
  ON public.financial_events FOR INSERT
  TO authenticated
  WITH CHECK (public.is_admin());

CREATE OR REPLACE FUNCTION public.admin_close_payroll_period(p_year integer, p_month integer)
RETURNS uuid
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path TO ''
